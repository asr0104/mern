...
module.exports = {
        ...
        postModel.find({}).sort({ posted_on: -1 }).exec((er, posts) => {
            if (er || posts == null) {
                ...
                for (let post of posts) {
                    postsList.push({ id: post._id, content: post.content, comments: post.comments, likes: post.likes, posted_on: post.posted_on, postedBy: post.email })
                }
                res.json({
                    error: false,
                    message: "Post found.",
    ...
    addNewComment: function (req, res, next) {
        ...
        postModel.findByIdAndUpdate(req.body.post_id, { $push: { comments: comment } }, { new: true ,useFindAndModify: false},function (err, post) {
            if (err || post == null) {
                return res.status(400).json({
                    error: true,
